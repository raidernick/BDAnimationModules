...
class BDFSnodeLoader
{
    ...
    public BDFSnodeLoader(Part _part, string _moduleName, string _moduleID, string _nodeName, string _valueName)
    {
        part = _part;
    ...
    public void debugMessage(string input)
    {
        if (debugMode)
        {
            BDAnimationModules.info.BDFSdebugMessages.Post("ProcessNodeAsStringList: " + input, true, 5f);
        }
    }
