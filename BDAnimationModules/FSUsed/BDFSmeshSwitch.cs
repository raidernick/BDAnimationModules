...
namespace BDAnimationModules.customization
{
    public class BDFSmeshSwitch : PartModule //, IPartCostModifier
    {
        [KSPField]
        ...
        private info.BDFSdebugMessages debug;
        ...
        private void parseObjectNames()
        {
            ...
            if (objectBatchNames.Length < 1)
                debug.debugMessage("BDFSmeshSwitch: Found no object names in the object list");
            else
            {
                ...
                for (int batchCount = 0; batchCount < objectBatchNames.Length; batchCount++)
                {
                    ...
                    for (int objectCount = 0; objectCount < objectNames.Length; objectCount++)
                    {
                        ...
                        if (newTransform != null)
                        {
                            ...
                            debug.debugMessage("BDFSmeshSwitch: added object to list: " + objectNames[objectCount]);
                        }
                        else
                        {
                            debug.debugMessage("BDFSmeshSwitch: could not find object " + objectNames[objectCount]);
                        }
                    }
        ...
        private void switchToObject(int objectNumber, bool calledByPlayer)
        {
            ...
            if (updateSymmetry)
            {
                ...
                {
                    BDFSmeshSwitch[] symSwitch = part.symmetryCounterparts[i].GetComponents<BDFSmeshSwitch>();
        ...
        public void initializeData()
        {
            if (!initialized)
            {
                debug = new info.BDFSdebugMessages(debugMode, "BDFSmeshSwitch");
