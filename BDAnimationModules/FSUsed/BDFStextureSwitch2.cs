...
using System.Text;
using UnityEngine;
using BDAnimationModules.info;

namespace BDAnimationModules.customization
{
    public class BDFStextureSwitch2 : PartModule
    {
        [KSPField]
        ...
        public void useTextureAll(bool calledByPlayer)
        {
            ...
            if (updateSymmetry)
            {
                for (int i = 0; i < part.symmetryCounterparts.Count; i++)
                {
                    ...
                    BDFStextureSwitch2[] symSwitch = part.symmetryCounterparts[i].GetComponents<BDFStextureSwitch2>();
        ...
        private void initializeData()
        {
            if (!initialized)
            {
                debug = new BDFSdebugMessages(debugMode, "BDFStextureSwitch2");
